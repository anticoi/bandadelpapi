...
// Helper: normalize image filename to avoid duplicated "fotos/" prefix
function getImagePath(filename) {
    if (!filename) return '';
    if (filename.startsWith('fotos/') || filename.startsWith('http')) return filename;
...
function renderPagination(totalPages) {
    ...
    `;
    prevBtn.className = 'w-10 h-10 rounded-full bg-amber-400/20 border border-amber-400/40 flex items-center justify-center text-amber-400 hover:bg-amber-400/30 transition-all disabled:opacity-30 disabled:cursor-not-allowed';
    ...
    `;
    nextBtn.className = 'w-10 h-10 rounded-full bg-amber-400/20 border border-amber-400/40 flex items-center justify-center text-amber-400 hover:bg-amber-400/30 transition-all disabled:opacity-30 disabled:cursor-not-allowed';
...
function createPageButton(pageNum) {
    ...
    if (pageNum === currentPage) {
        btn.className = 'w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-black font-bold transition-all';
    } else {
        btn.className = 'w-10 h-10 rounded-full bg-[#0c0612] border border-amber-400/30 flex items-center justify-center text-gray-300 hover:bg-amber-400/10 hover:text-amber-400 transition-all';
    }
...
function openLightbox(globalIndex) {
    ...
    lightbox.innerHTML = `
        <button class="absolute top-4 right-4 text-white hover:text-amber-400 transition-colors z-10" onclick="closeLightbox()">
            <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
        </button>
        <button class="absolute left-4 top-1/2 transform -translate-y-1/2 text-white hover:text-amber-400 transition-colors z-10" onclick="navigatePhoto(-1)">
            <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>
        <button class="absolute right-4 top-1/2 transform -translate-y-1/2 text-white hover:text-amber-400 transition-colors z-10" onclick="navigatePhoto(1)">
            <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
        </button>
        <div class="max-w-5xl max-h-[90vh] relative">
            ...
            <div class="text-center mt-4">
                <h3 class="text-2xl font-bold text-amber-400 mb-2">${photo.title}</h3>
